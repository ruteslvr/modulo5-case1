...
function Clientes() {
  ...
  function createPost(project){ // Talvez mudar project para imovel
    ...
    fetch('http://localhost:5000/compradores', {
      ...
      console.log(data)
      //redirect
      navigate('/dashcompradores', {message:'Evento inserido com sucesso!'})
    })
  ...
  return (
    <div className={styles.compradores_container}>
      ...
      <FormClient handleSubmit={createPost} btnText={"Inserir evento"} /> {/*Essa prop(btnText) irá mudar para de acordo com o método. Esse método será um POST. */}
    </div>
  );
